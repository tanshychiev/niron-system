...
def customer_search(request):
    # Customer search is used both by the Customer module and by Order
    # Create/Edit. Do not expose customer phone/location data to every
    # authenticated account; require at least one relevant permission.
    can_search = (
        request.user.has_perm("customers.view_customer")
        or request.user.has_perm("orders.add_order")
...
def customer_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        customer.save(
            update_fields=["name", "phone", "location", "updated_at"]
        )
